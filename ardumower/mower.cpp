...
#define pinChargeRelay 46        // charge current sensor  
...
Mower::Mower(){
  ...
  motorReverseTime    = 1000;  // max. reverse time (ms)
  ...
  motorLeftPID.Kp = motorRightPID.Kp = 0.5;
  ...
  motorMowRPM        = 3333;   // motor mower RPM (only for cutter modulation)
  ...
  batSenseZero       =0.0;        // battery volt sense zero point
  batFull          =29.3;      // battery reference Voltage (fully charged)
  chgSenseZero      = 0.0;       // charge current sense zero point
  chgFactor       = 17.42;     // charge current conversion factor
  // ------  charging station ---------------------------
...
void Mower::setup(){
 ...
 // drops
  ...
  // ADC
  ADCMan.init();
  ADCMan.setCapture(pinChargeCurrent, 10, true);//Aktivierung des LaddeStrom Pins beim ADC-Managers      
